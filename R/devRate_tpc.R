...
#' @param lowTempLim The minimum temperature for the metrics (default = 0 degree celsius).
...
dRGetMetricsInfo <- function(eq, prec = 0.1, lowTempLim = 0, highTempLimit = 60, devLimit = 0.01, devThresh = 0.1,
  ...
  dfMetrics <- sapply(1:nrow(parameters), function(j){
    ...
    cTXmin <- max(devT[devRT <= max(devRT, na.rm = TRUE) * devThresh & devT < topt], na.rm = TRUE)
    cTXmax <- min(devT[devRT <= max(devRT, na.rm = TRUE) * devThresh & devT > topt], na.rm = TRUE)
    tolRange <- cTmax - cTmin
    tolXRange <- cTXmax - cTXmin
    tbMin <- max(devT[devRT <= max(devRT, na.rm = TRUE)/2 & devT < topt], na.rm = TRUE)
    tbMax <- min(devT[devRT <= max(devRT, na.rm = TRUE)/2 & devT > topt], na.rm = TRUE)
    tbr <- tbMax - tbMin
    tsm <- cTmax - topt
    tsfX <- cTXmax - topt
