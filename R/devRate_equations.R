...
analytis_77 <- list(eq = rT ~ aa * (T - Tmin)^bb * (Tmax - T)^cc ,
                    ...
                    startVal = data.frame(
                      ...
                      param = list(
                        ...
                        bb = c(1.3945, 1.7766),
                        cc = c(0.0577, 0.1740),
                        Tmin = c(7.9601, 4.9125),
                        Tmax = c(35.0276, 33.0781)
                      ),
                      ref = c(rep("Kontodimas et al. 2004"))
                    ),
                    com = '',
...
hilbertLogan_83 <- list(eq = rT ~ phi * ((T^2 / (T^2 + aa^2)) - exp((Tmax - T)/deltaT)) ,
                        eqAlt = "phi * ((x^2 / (x^2 + aa^2)) - exp((Tmax - x)/deltaT))",
...
briere1_99 <- list(eq = rT ~ aa * T * (T - Tmin) * (Tmax - T)^(1 / 2) ,
                   ...
                   startVal = data.frame(
                     ...
                     param = list(
                       aa = c(0, 0),
                       Tmin = c(10.3132, 9.1307),
                       Tmax = c(35.5970, 36.2159)
                     ),
                     ref = c(rep("Kontodimas et al. 2004"))
                   ),
                   com = '',
...
kontodimas_04 <- list(eq = rT ~ aa * (T - Tmin)^2 * (Tmax - T) ,
                      ...
                      startVal = data.frame(
                        ...
                        param = list(
                          aa = c(0.0001, 0.0001),
                          Tmin = c(8.5021, 7.4083),
                          Tmax = c(45.4570, 41.6658)
                        ),
                        ref = c(rep("Kontodimas et al. 2004"))
                      ),
                      com = "",
